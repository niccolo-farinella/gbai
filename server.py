#!/usr/bin/env python3
"""
ai_player_v5_strategist_py_nav_api.py — Strategist + Python Navigator + API Supervisor.

Features:
- Strategist (Llama): Gestisce la storia e gli obiettivi a lungo termine.
- Navigator (Python): Calcola i percorsi A* deterministicamente.
- API Supervisor (FastAPI): Riceve comandi HTTP, interrompe l'agente e forza nuovi obiettivi.

Usage:
  python3 ai_player_v5_complete.py
  
  Poi invia comandi via POST a http://localhost:8000/api/instruct
  JSON Body: {"instruction": "Vai al centro pokemon"}
"""
...
app = FastAPI(title="Pokemon AI Controller")
agent_ref: Optional['AgentSystemV5'] = None  # Global ref to inject commands

class InstructionRequest(BaseModel):
    instruction: str

@app.post("/api/instruct")
def send_instruction(req: InstructionRequest):
    """
    Invia un comando prioritario all'agente (Supervisor).
    Interrompe l'azione corrente e forza l'interpretazione del comando.
    """
    if agent_ref is None:
        raise HTTPException(status_code=503, detail="Agent non ancora avviato.")
    
    # Questo metodo lancia un thread che chiama Ollama (Supervisor)
    agent_ref.set_user_override(req.instruction)
    return {"status": "processing", "instruction": req.instruction}

@app.get("/api/status")
def get_status():
    if agent_ref is None or agent_ref.lua.latest_state is None:
        return {"status": "booting"}
    
    st = agent_ref.lua.latest_state
    return {
        "frame": st.frame,
        "map": st.map_name,
        "coords": {"x": st.x, "y": st.y},
        "current_goal": agent_ref.current_goal,
        "mode": st.mode,
        "queue_size": len(agent_ref.plan_queue)
    }
...
def _extract_json_object(text: str) -> Any:
    ...
    try:
        ...
        pass
    # Tentativo di recuperare JSON tra blocchi markdown o parentesi
    start = s.find("{")
    end = s.rfind("}")
...
class StrategistDecision:
    ...
    def parse(payload_text: str) -> "StrategistDecision":
        ...
        if button and button not in {"A", "B", "START", "SELECT", "UP", "DOWN", "LEFT", "RIGHT"}:
            button = "A"
...
def supervisor_prompt(user_text: str, semantic_keys: List[Dict[str, Any]]) -> str:
    """
    Prompt dedicato per il 'Supervisor':
    Traduce un comando in linguaggio naturale in un JSON tecnico.
    NON conosce lo stato del gioco, si occupa solo di intent matching.
    """
    keys_list = [k["key"] for k in semantic_keys]
    
    return json.dumps({
        "role": "SYSTEM_TRANSLATOR",
        "instruction": "You are a strict JSON command translator. Convert the USER_INPUT into a JSON action.",
        "user_input": user_text,
        "available_semantic_locations": keys_list,
        "output_schema": {
            "intent": "GO_TO | PRESS | WAIT",
            "goal": "Must be one of available_semantic_locations OR null",
            "button": "A | B | START | SELECT | UP | DOWN | LEFT | RIGHT | null",
            "repeat": "integer (1-10)"
        },
        "rules": [
            "Output ONLY valid JSON. No markdown, no conversational text.",
            "If user wants to move/travel, find the closest matching 'goal' key.",
            "If user wants to stop/pause, use intent='WAIT'.",
            "If user wants to battle/interact, use intent='PRESS' with button='A'."
        ]
    }, ensure_ascii=False)
...
class AgentSystemV5:
    def __init__(self):
        ...
        # --- MODIFICA FIX BATTAGLIA ---
        self.config_ignore_battle = False  # Flag per ignorare la battaglia se buggata
        self.battle_idle_frames = 0        # Contatore per rilevare timeout UI
        self.last_map_id = (-1, -1)        # Per resettare il flag al cambio mappa
        self.pending_user_override: Optional[str] = None
    ...
    # --- API / SUPERVISOR LOGIC ---

    def _process_user_instruction(self, instruction: str):
        """
        Eseguita in thread separato. Chiama Ollama come 'Supervisor' (stateless)
        per tradurre il comando utente in un'azione JSON e forzare l'aggiornamento.
        """
        print(f"[SUPERVISOR] Analisi comando utente: '{instruction}'...")
        
        # 1. Costruiamo il prompt 'traduttore'
        prompt = supervisor_prompt(instruction, self.semantic_compact)
        ...
        # 3. Pulizia risposta (estrazione JSON)
        ...
        dec = StrategistDecision.parse(clean_json)
        print(f"[SUPERVISOR] Tradotto in: {dec}")

        # 4. Applicazione forzata dell'ordine
        self.plan_queue.clear()
        self.last_sent_input = None # reset stuck detection
        
        if dec.intent == "GO_TO" and dec.goal:
            self.current_goal = dec.goal
            self.current_policy = self._normalize_policy(dec.policy)
            # Reset timer per forzare ricalcolo immediato del navigatore
            self.last_nav_time = 0 
            print(f"[SUPERVISOR] Nuovo obiettivo forzato: {dec.goal}")
        
        elif dec.intent in {"PRESS", "BATTLE_AUTO"}:
            self.current_goal = None 
            self.plan_queue.append((dec.button or "A", DEFAULT_BUTTON_HOLD_FRAMES * dec.repeat))
            print(f"[SUPERVISOR] Azione manuale forzata: {dec.button} x{dec.repeat}")

        elif dec.intent == "WAIT":
            self.current_goal = None
            self.plan_queue.clear()
            print("[SUPERVISOR] Agente messo in pausa.")

    def set_user_override(self, instruction: str):
        # Lancia il processo di traduzione in background per non bloccare la risposta HTTP
        threading.Thread(target=self._process_user_instruction, args=(instruction,), daemon=True).start()
    ...
    def run(self):
        ...
        server_thread = threading.Thread(
            target=uvicorn.run, 
            args=(app,), 
            kwargs={"host": "0.0.0.0", "port": API_PORT, "log_level": "error"}, 
            daemon=True
        )
        server_thread.start()
        print(f"[api] FastAPI server running on http://127.0.0.1:{API_PORT}")
        ...
        while True:
            ...
            # 1. Reset di sicurezza al cambio mappa
            # Se cambiamo mappa, resettiamo eventuali flag di ignore battle
            current_map_id = (st.map_group, st.map_num)
            if self.last_map_id != current_map_id:
                if self.config_ignore_battle:
                    print(f"[AUTO-FIX] Cambio mappa rilevato. Reset stato battaglia.")
                self.config_ignore_battle = False
                self.last_map_id = current_map_id

            # 2. Calcolo Mode Effettivo
            # Se config_ignore_battle è True, forziamo l'Overworld anche se st.mode dice BATTLE
            effective_mode = st.mode
            if self.config_ignore_battle:
                effective_mode = "OVERWORLD"
            ...
            # BATTLE HANDLING
            # Usiamo effective_mode invece di st.mode
    ...
    def _handle_battle(self, st: GameState):
        """
        Gestione Battaglia Deterministica.
        Rileva 'Ghost Battles' analizzando l'incoerenza degli stati UI/Controllo.
        """
        now = time.time()

        # CHECK 1: Contraddizione Logica (Immediato)
        # Se il gioco dice che abbiamo il controllo del personaggio (flag overworld)
        # ma siamo in mode=BATTLE, allora il mode=BATTLE è un errore di memoria.
        if st.control_enabled:
            print(f"[STATE CHECK] Incoerenza rilevata: Mode=BATTLE ma Control=ENABLED. Force Overworld.")
            self.config_ignore_battle = True
            self.battle_idle_frames = 0
            return

        # CHECK 2: Timeout Interazione (3 secondi)
        # Una vera battaglia DEVE avere o un Menu (Fight/Bag) o una Textbox attivi.
        # Se per 180 frame non c'è nulla, siamo bloccati nell'Overworld con un flag errato.
        has_active_ui = st.menu_open or st.textbox_open
        
        if not has_active_ui:
            self.battle_idle_frames += 1
        else:
            self.battle_idle_frames = 0 # Reset se appare un menu o testo

        if self.battle_idle_frames > 180: # ~3 secondi a 60fps
            print(f"[STATE CHECK] Timeout UI in battaglia ({self.battle_idle_frames} frames senza menu/testo). Force Overworld.")
            self.config_ignore_battle = True
            self.battle_idle_frames = 0
            return

        # --- ESECUZIONE STANDARD ---
        # Se superiamo i check, è una battaglia vera. Premiamo A normalmente.
    ...
    def _press(self, btn: str, st: GameState, repeat: int = 1):
        ...
        if btn not in {"A", "B", "START", "SELECT", "UP", "DOWN", "LEFT", "RIGHT"}:
            btn = "A"
