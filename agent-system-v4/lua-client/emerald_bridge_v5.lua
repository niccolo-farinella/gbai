-- emerald_bridge_v5.lua
-- Bridge mGBA <-> ai_player_v4.py
...
-- OPTIONAL UI FLAGS (set addresses if known; otherwise keep nil to disable)
local ADDR_UI_TEXTBOX = 0x03000AE8    -- e.g. textbox state/nonzero when dialog box is open
local ADDR_UI_MENU    = 0x020387F4    -- e.g. menu state/nonzero when start/menu is open
local ADDR_UI_CONTROL = 0x0203761E    -- e.g. player control enabled flag (0/1)
...
local logBuffer = console:createBuffer("emerald_bridge_v5")
...
local function connect_socket()
    ...
    log(string.format("emerald_bridge_v5: connecting to %s:%d", HOST, PORT))

    local ok, res = pcall(socket.connect, HOST, PORT)
    if not ok then
        log("emerald_bridge_v5: connect error: " .. tostring(res))
        connected = false
        sock = nil
        return
    end
    ...
    sock = res
    if not sock then
        log("emerald_bridge_v5: socket.connect returned nil")
        connected = false
        return
    end
    ...
    connected = true
    log("emerald_bridge_v5: connected")
end
...
local function send_line(line)
    ...
    if not ok then
        log("emerald_bridge_v5: send failed: " .. tostring(err))
        connected = false
        sock = nil
    end
end
...
local function get_state_json()
    ...
    -- Formato esattamente come ai_player_v4.py si aspetta
    local frame = emu:currentFrame()

    -- Optional UI flags (safe defaults if addresses are unknown)
    local textbox_open = false
    local menu_open = false
    local control_enabled = true

    if ADDR_UI_TEXTBOX then
        textbox_open = (emu:read8(ADDR_UI_TEXTBOX) ~= 0)
    end
    if ADDR_UI_MENU then
        menu_open = (emu:read8(ADDR_UI_MENU) ~= 0)
    end
    if ADDR_UI_CONTROL then
        control_enabled = (emu:read8(ADDR_UI_CONTROL) ~= 0)
    end

    return string.format(
        '{"frame":%d,"mode":"%s",' ..
        '"ui":{"textbox_open":%s,"menu_open":%s,"control_enabled":%s},' ..
        '"map":{"group":%d,"num":%d,"x":%d,"y":%d},' ..
        '"party":{"hp":%d,"max_hp":%d}}\n',
        frame, mode,
        tostring(textbox_open), tostring(menu_open), tostring(control_enabled),
        g, n, x, y, hp, mhp
    )
end
...
local function handle_input_line(line)
    ...
    if k == "NONE" then
        ...
        input_frames_left = 0
        emu:setKeys(0)
        log("emerald_bridge_v5: input NONE -> release keys")
        return
    end
    ...
    if not mask then
        ...
        log("emerald_bridge_v5: unknown key '" .. k .. "'")
        return
    end

    current_keys_mask  = mask
    input_frames_left = frames
    log(string.format("emerald_bridge_v5: INPUT %s (%d frames)", k, frames))
end
...
local function poll_commands()
    ...
    if not data then
        if partial and #partial > 0 then
            ...
            log("emerald_bridge_v5: receive error: " .. tostring(recv_err))
            connected = false
            sock = nil
            return
        end
...
log("emerald_bridge_v5: script loaded, waiting for backend.")
